...
assert.deepEqual(OB_CONTENT_SLIDES, [2, 4, 5]);

// Content order — never Welcome(2) → Age(3)
assert.equal(nextContentSlide(2), 4, 'Welcome advances to Privacy');
assert.equal(nextContentSlide(4), 5, 'Privacy advances to Tell Me About You');
assert.equal(nextContentSlide(5), null);
...
console.log('  New user: Welcome → Privacy → Age (if needed) → Tell Me About You → Home');
